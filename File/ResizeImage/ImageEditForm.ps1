...
function CreateClipControls() {
  ...
  # クリップ幅入力コントロール
  $script:clipSizeUpDown = New-Object System.Windows.Forms.NumericUpDown
  $clipSizeUpDown.Minimum = 0
  $clipSizeUpDown.Maximum = 100
  $clipSizeUpDown.Value = 10
  $clipSizeUpDown.Increment = 10
  $clipSizeUpDown.Add_ValueChanged({
      Write-Host "NumericUpDown: $($this.Value)"
    })
  $buttonPanel.Controls.Add($clipSizeUpDown)
...
# グレースケール用のUIコントロールを作成
function CreateGrayControls() {
  # 見出しラベル
