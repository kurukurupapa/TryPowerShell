...
.EXAMPLE
powershell -File MaskSjis2.ps1 in.csv out.csv
.\File\Masking\MaskSjis2.ps1 .\File\Masking\SampleInput\sjis.csv .\File\Masking\SampleOutput\sjis2.csv
#>
...
function Convert-Field {
    ...
    # Unicodeコード：
    #   0x2160-0x2169（Ⅰ-Ⅹ）、～0x216B（??）
    #   0x2170-0x2179（ⅰ-ⅹ）、～0x217B（??）
    # Unicodeカテゴリ：
    ...
    $text = $text -replace '\p{IsNumberForms}', 'Ⅲ'
    ...
    $text = $text -replace '\p{IsKatakana}', 'ア'
    ...
    $text = $text -replace '\p{IsHiragana}', 'あ'
    ...
    $text = $text -replace '\p{IsCJKUnifiedIdeographs}', '亜'
    return $text
}
