<#
個人情報を含むCSVデータを、マスキングしたCSVデータに変換するスクリプトです。

.SYNOPSIS
CSVデータの個人情報カラムをマスキングした新しいCSVデータを出力します。
...
.EXAMPLE
Get-Content in.csv | Mask_UTF8_StdInOut.ps1 | Set-Content out.csv
Get-Content -Encoding UTF8 in.csv | Mask_UTF8_StdInOut.ps1 | %{ [Text.Encoding]::UTF8.GetBytes($_+"`r`n") } | Set-Content -Encoding Byte out.csv
#>
...
Write-Host "マスキング済みCSVを標準出力に出力しました。"
$endTime = Get-Date
