...
.EXAMPLE
powershell -File Mask_UTF8_IOStream.ps1 in.csv out.csv
.\File\Masking\Mask_UTF8_IOStream.ps1 .\File\Masking\SampleInput\utf8_CRLF.csv .\File\Masking\tmp.csv
#>
...
try {
    while ($null -ne ($line = $reader.ReadLine())) {
        ...
        if ($first) {
            ...
            $count++
            $maskStartTime = Get-Date
            for ($i = 0; $i -lt $columns.Count; $i++) {
                if ($columnIndexes -contains $i) {
                    $columns[$i] = Convert-Field $columns[$i]
                }
            }
            $maskEndTime = Get-Date
            $maskTime += $maskEndTime - $maskStartTime
        }
        $csvLine = $columns -join ','
        $writer.Write($csvLine + $newLineCode)
    }
}
finally {
    $reader.Close()
